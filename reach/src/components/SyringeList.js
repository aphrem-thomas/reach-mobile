...
class SyringeList extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
    ...
    render() {
        ...
        const tableData = [];
        if(this.props.Syringe){
        this.props.Syringe.map((item) => {
            let temp = [];
            temp.push(item.syringeType);
        ...
        return (
            <View style={styles.container}>
                <Text style={{ fontSize: 20 }}>Syringe Details</Text>
                    <View style={{ flex: 1}} >
...
export default connect(mapStateToProps)(SyringeList);